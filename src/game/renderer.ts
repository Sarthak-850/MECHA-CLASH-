...
export class GameRenderer {
  ...
  public render(
    ...
    theme: 'CYAN_MATRIX' | 'CRIMSON_FORGE' | 'NEON_PURPLE' | 'GOLD_CORE' = 'CYAN_MATRIX'
  ) {
    this.animTime += dt;
    const ctx = this.ctx;
  ...
  private drawArenaFloor(theme: string) {
    ...
    if (theme === 'CRIMSON_FORGE') gridColor = 'rgba(239, 68, 68, 0.05)';
    if (theme === 'NEON_PURPLE') gridColor = 'rgba(168, 85, 247, 0.05)';
    if (theme === 'GOLD_CORE') gridColor = 'rgba(234, 179, 8, 0.05)';

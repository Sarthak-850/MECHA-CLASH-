...
export class GameEngine {
  ...
  public startMatch(mode: GameMode, diff: DifficultyLevel = 'NORMAL', level: number = 1) {
    ...
    this.aiController.setConfig(this.getEffectiveAIConfig());
  ...
  private getEffectiveAIConfig(): AIDifficultyConfig {
    ...
    if (this.gameMode === 'ENDLESS' && this.currentLevel > 1) {
      // Smooth gradual enhancement
      const bonus = Math.min(0.2, (this.currentLevel - 1) * 0.015);
      base.moveSpeedMultiplier += bonus;
      base.predictionStrength = Math.min(1.0, base.predictionStrength + bonus);
    }
    return base;
  }
  ...
  public setupArenaHazards() {
    this.obstacles = [
      // 4 symmetrical tactical cyber pillars
    ...
    // Decide hazards based on difficulty or campaign configuration
